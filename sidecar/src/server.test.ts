...
test("deployment scans the immutable artifact and never treats scanner failure as success", async () => {
	...
	const h = harness({ scanArtifact: async (html) => { scanned.push(html); return { allowed: false, reason: "a sensitive value that must not be reflected" }; } });
	let site = await create(h);
	const html = "<!doctype html><h1>Private draft</h1>";
	site = await save(h, site, html);
	const response = await h.admin(`/projects/${site.id}/deploy`, "POST", { versionId: site.versions[0]?.id, expectedRevision: site.revision });
	...
	const report = await (await h.admin(`/projects/${site.id}/security`)).json() as { status: string; artifactDigest: string };
	expect(report.status).toBe("blocked");
	expect(report.artifactDigest).toMatch(/^[a-f0-9]{64}$/);
	const unavailable = harness({ scanArtifact: undefined });
	const saved = await save(unavailable, await create(unavailable, "scanner-off"));
	expect((await unavailable.admin(`/projects/${saved.id}/deploy`, "POST", { versionId: saved.versions[0]?.id, expectedRevision: saved.revision })).status).toBe(422);
});
