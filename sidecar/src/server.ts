			...
			guarded(async () => {
				...
				if (path === "/status" && request.method === "GET") {
					return json({
						...
						agentAvailable: Boolean(services.runResetAction),
				...
				if (action === "settings" && request.method === "PUT") {
					...
					if (
						input.settings.agentActions &&
						site.template !== "reset-calendar"
					) {
						return fail(
							"invalid_input",
							"Owner reset actions require the Reset calendar starter"
						);
					}
					if (input.settings.agentActions && !services.runResetAction) {
						return fail(
							"unavailable",
							"Ryu agent execution is not connected",
							503
						);
					}
					if (input.settings.agentActions && !services.identity) {
						return fail(
							"unavailable",
							"Connect platform identity before enabling owner reset actions",
							503
						);
					}
	...
	const visitor = Bun.serve({
		...
		fetch: (incoming, server) =>
			guarded(async () => {
				...
				const request =
					url.href === incoming.url ? incoming : new Request(url, incoming);
				...
				if (url.pathname === "/_ryu/config" && request.method === "GET") {
					return json({
						...
						resetActions:
							site.template === "reset-calendar" &&
							site.settings.agentActions &&
							Boolean(services.runResetAction),
...
async function visitorData(
	...
	if (url.pathname === "/_ryu/reset" && request.method === "POST") {
		if (site.template !== "reset-calendar") {
			return fail(
				"not_found",
				"Reset actions are not enabled for this Site",
				404
			);
		}
		if (!site.settings.agentActions) {
			return fail(
				"disabled",
				"The Site owner has not enabled reset actions",
				403
			);
		}
		if (!(user && isOwner(site, user))) {
			return fail(
				"forbidden",
				"Only the signed-in Site owner can run reset actions",
				403
			);
		}
		if (!services.runResetAction) {
			return fail("unavailable", "Ryu agent execution is not connected", 503);
		}
		const input = z
			.object({ provider: resetProvider })
			.strict()
			.parse(await body(request, 10_000));
		return json(
			await services.runResetAction({
				ownerId: user.id,
